          ...
          {notifications.map((n) => {
            ...
            return (
              <div
                ...
                className={`flex items-start gap-4 px-5 py-4 transition-colors cursor-pointer ${n.link ? "hover:bg-white/[0.03]" : "cursor-default"} ${!n.read ? "bg-white/[0.015]" : ""}`}
              >
                <div className={`flex items-center justify-center w-10 h-10 rounded-xl shrink-0 mt-0.5 ${style.icon}`}>
                  <NotificationIcon type={n.type} />
                </div>
                <div className="flex-1 min-w-0">
                  <p className={`text-sm leading-snug ${!n.read ? "font-semibold text-white" : "font-medium text-white/70"}`}>{n.title}</p>
                  <p className="text-sm text-gray-500 mt-0.5 leading-snug">{n.message}</p>
                  <p className="text-xs text-gray-600 mt-1.5">{formatNotificationTime(n.createdAt)}</p>
                </div>
