...
function NavLink({ href, label, icon, badge, unreadCount, isActive }: {
  ...
  return (
    <Link
      href={href}
      className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-colors text-[15px] leading-snug ${
    ...
    >
      ...
      {badge && unreadCount && unreadCount > 0 ? (
        <span className="ml-auto bg-[#FFC428] text-[#010101] text-[12px] font-bold w-5 h-5 rounded-full flex items-center justify-center shrink-0">
...
export default function Sidebar({ onClose }: { onClose?: () => void }) {
  ...
  if (!session) {
    return (
      <>
        <aside className="w-64 bg-[#0B298F] h-full flex flex-col">
          <div className="border-b border-white/10 flex items-center px-4 py-3 gap-2">
            ...
            <span className="text-xs font-semibold italic text-[#FFC428] uppercase">driver</span>
          </div>
  ...
  return (
    <aside className="w-64 px-4 py-11 bg-[#0B298F] h-full flex flex-col">
      {/* Logo */}
      ...
      <div className="px-4">
        ...
        >
          ...
          <div className="min-w-0">
            <p className="text-[14px] font-normal text-[#ffffff] truncate leading-tight">
              {session?.user?.name || "Kierowca"}
            </p>
            <p className="text-[12px] text-[rgba(255,255,255,0.7)] truncate">
              {session?.user?.email}
            </p>
          </div>
        </Link>
