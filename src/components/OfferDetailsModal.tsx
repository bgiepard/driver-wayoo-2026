      ...
      <div className="relative w-full max-w-2xl mx-4 rounded-xl border border-gray-200 bg-white shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 custom-scrollbar">
          <div className="px-6 py-5 space-y-4">
            ...
            {offer.passengerContact && (
              <div className="rounded-lg border border-[#e0e7ff] bg-[#f5f7ff] p-4">
                <h3 className="text-xs font-medium text-[#6366f1] uppercase tracking-wider mb-3">Dane kontaktowe pasażera</h3>
                <div className="flex flex-col gap-2.5">
                  {offer.passengerContact.name && (
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-[#e0e7ff] flex items-center justify-center shrink-0">
                        <svg className="w-4 h-4 text-[#6366f1]" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0" />
                        </svg>
                      </div>
                      <span className="text-sm font-semibold text-gray-900">{offer.passengerContact.name}</span>
                    </div>
                  )}
                  {offer.passengerContact.phone && (
                    <div className="flex items-center gap-3">
                      ...
                      <a href={`tel:${offer.passengerContact.phone}`} className="text-sm font-bold text-[#16a34a] hover:underline">
                        {offer.passengerContact.phone}
                      </a>
                    </div>
                  )}
                  {offer.passengerContact.email && (
                    <div className="flex items-center gap-3">
                      ...
                      <a href={`mailto:${offer.passengerContact.email}`} className="text-sm text-[#ca8a04] hover:underline break-all">
                        {offer.passengerContact.email}
                      </a>
                    </div>
                  )}
                  {!offer.passengerContact.name && !offer.passengerContact.phone && !offer.passengerContact.email && (
                    <p className="text-xs text-gray-400">Brak danych kontaktowych pasażera.</p>
                  )}
                </div>
              </div>
            )}
